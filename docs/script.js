...
function testCaioVoice() {
  // 1) Confirm the button is actually wired
  alert("CAIO voice test button wired ✅");
  ...
  if (audio) {
    ...
    if (playPromise && typeof playPromise.then === "function") {
      playPromise
        .then(() => {
          // Playing successfully, we’re done
          console.log("✅ MP3 audio playing");
        })
        .catch(err => {
          alert("MP3 error: " + err.message + "\nTrying TTS fallback next…");
          playCaioTtsFallback();
        });
    } else {
      // Older browser – just attempt play
      try {
        audio.play();
      } catch (e) {
        alert("MP3 play threw an error, trying TTS fallback…");
        playCaioTtsFallback();
      }
    }
  ...
  // 3) If no audio element, go straight to TTS
  alert("No audio element found, trying TTS fallback…");
  playCaioTtsFallback();
}

// Simple TTS fallback using speechSynthesis
function playCaioTtsFallback() {
  ...
  const text = `
    CAIO preview online.
    This is a simple voice test for the executive role-play system.
  `.trim();
